...
// Only filter pathological directories that would blow up the tree.
// Dot-files / dot-directories are shown by default — the user wants full visibility.
...
// Default to depth 1 for lazy-load behaviour (folder click fetches its own children).
const DEFAULT_DIRECTORY_DEPTH = 1
      ...
      GET: async ({ request }) => {
        ...
        try {
          ...
          if (action === 'read') {
            ...
            if (st.isDirectory()) {
              return json({ error: 'Is a directory' }, { status: 400 })
            }
            // Hard size cap — protects memory on accidental binary opens (state.db etc).
            const MAX_READ_BYTES = 8 * 1024 * 1024 // 8 MiB
            if (st.size > MAX_READ_BYTES) {
              return json(
                {
                  error: `File too large (${Math.round(st.size / 1024 / 1024)} MiB) — limit is ${Math.round(MAX_READ_BYTES / 1024 / 1024)} MiB`,
                },
                { status: 413 },
              )
            }
            ...
            // Detect binary by NUL byte in the first sample
          ...
          const tree = await readDirectory(resolvedPath, 0, {
            maxDepth: maxDepthParam ?? DEFAULT_DIRECTORY_DEPTH,
