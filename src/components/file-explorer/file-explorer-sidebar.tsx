...
type FileExplorerSidebarProps = {
  ...
  /** Called when a file (not a folder) is clicked. */
...
async function fetchFileTree(targetPath: string = ''): Promise<Array<FileEntry>> {
...
/** Walk `tree` and merge `children` into the folder at `targetPath`. */
function attachChildren(
  ...
  return tree.map((entry) => {
    ...
    if (entry.type === 'folder' && entry.children?.length) {
      return { ...entry, children: attachChildren(entry.children, targetPath, children) }
    }
    return entry
  })
}
...
export function FileExplorerSidebar({
  ...
  const toggleFolder = useCallback(
    async (entry: FileEntry) => {
      ...
      const wasExpanded = expanded.has(pathValue)
      setExpanded((prev) => {
      ...
      // Lazy-fetch children the first time a folder is expanded
      if (!wasExpanded && entry.children === undefined) {
        ...
        try {
          const children = await fetchFileTree(pathValue)
          setEntries((prev) => attachChildren(prev, pathValue, children))
        } catch {
          // ignore, folder stays expanded but empty
        } finally {
          setLoadingPaths((prev) => {
            const next = new Set(prev)
  ...
  const handleFileClick = useCallback(
    (entry: FileEntry) => {
      ...
      if (onFileOpen) {
        ...
        // Fallback for callers that haven't wired onFileOpen
        onInsertReference(buildReference(entry.path))
        setPreviewPath(entry.path)
      }
    },
    [onFileOpen, onInsertReference, toggleFolder],
  )
  ...
  const renderEntry = useCallback(
    (entry: FileEntry, depth: number) => {
      ...
      const isLoadingFolder = loadingPaths.has(entry.path)
      ...
      return (
        <div key={entry.path}>
          ...
          >
            ...
            {isLoadingFolder ? (
              <span className="ml-auto pr-2 text-[10px] text-primary-500">…</span>
            ) : null}
          </button>
          ...
          !isLoadingFolder ? (
            <div
              className="text-xs text-primary-400 italic py-1"
    ...
    [expanded, handleFileClick, isSearchActive, setContextMenu, loadingPaths],
  )
