...
import { ensureLogStoreStarted } from './log-store'

// Initialise log interception as early as possible (module load time)
...
export async function dashboardFetch(
  ...
  const shortPath = requestPath.replace(HERMES_DASHBOARD_URL, '')
  console.log(`[dashboard] ${method} ${shortPath} → ${res.status} (${ms}ms)`)
  return res
}
...
async function probeDashboard(): Promise<{ available: boolean; url: string }> {
  // Retry up to 2 times with backoff to handle transient unavailability
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      // Pre-warm the dashboard token so the first session API call doesn't pay the scrape cost
